...
describe('Inverted Index Test', () => {
  describe('Is createIndex properly called?', () => {
    it('should throw, `Improper arguement`', () => {
  ...
  describe('Is JSON content properly flattened in the method bookContent?', () => {
    const expected = 'I love being your developer Being a developer is one of my greatest dream '
      + 'in life Life is full of mystery Your student today will be your teacher tomorrow, its dancing time';
  ...
  describe('Checks if the `searchIndex` returns valid results', () => {
    ...
    invertedIndexTest.createIndex('book1.json', book1);
    ...
    it('Should return `try not in index`', () => {
      expect(InvertedIndex.searchIndex(indices, 'try.json', 'dancing')).toBe('try.json not in index');
  });
