...
describe('Inverted Index Test', function () {
  describe('Is createIndex properly called?', function () {
    ...
    it('should throw, `Malformed file`', function () {
      ...
      expect(invertedIndexTest.createIndex('string', _malformed2.default)).toThrow('Malformed file');
      expect(invertedIndexTest.createIndex('string', _bad2.default)).toThrow(Error('Malformed file'));
    });
  });
  describe('Checks If file is malformed', function () {
    ...
    it('should return `false` for valid JSON file', function () {
      expect(function () {
        _invertedIndex2.default.isFileMalformed(_valid2.default);
      }).toBe(false);
    });
  });
  describe('Check if array or token is formed from file', function () {
    ...
    it('Should return an array of unique token', function () {
      ...
      expect(function () {
        _invertedIndex2.default.arrayFromText(text2);
      }).toEqual(validTokens2);
    });
  });
  describe('Is JSON content properly flattened in the method bookContent?', function () {
    var expected = 'I love being a developer Being a developer is one of my greatest dream' + 'Life is full of mystery Your student today will be your teacher tomorrow, its dancing time';
  ...
  describe('Checks if the `searchIndex` returns valid results', function () {
    ...
    invertedIndexTest.createIndex('book1.json', _valid2.default);
